...
def _read_inst2class_raw(path: Path) -> dict | None:
    """Load sidecar JSON; tolerate empty / BOM / minor corruption (returns None on failure)."""
    try:
    ...
    def _load_inst2class(self, stem: str) -> dict[int, int] | None:
        ...
        if raw is None:
            if self._bad_inst2class_warn < 8:
                print(
                    f"Warning: missing or invalid inst2class JSON for {p.name} "
                    "(skipping class targets for this tile)"
                )
            self._bad_inst2class_warn += 1
            return None
        if not isinstance(raw, dict):
            if self._bad_inst2class_warn < 8:
                print(f"Warning: inst2class must be a JSON object for {p.name}")
            self._bad_inst2class_warn += 1
            return None
        out: dict[int, int] = {}
